...
export default function RecipeCard({arrayObject, relatedRecipes, arrayKey, typeHover="missingIngredients"}:{arrayObject:Recipe, relatedRecipes?:string, arrayKey:number, typeHover?:string}){
  const r = useRouter()
  ...
  return (
    <Flex id="what" position="relative" onMouseOver={()=>setMissingHover(arrayObject.title)} onMouseOut={()=>setMissingHover("")} bg="white" flexDir="column" width="170px" height="180px" alignItems="center" borderRadius="25px" p="15px" key={arrayKey} onClick={()=>handleRecipeInformation(arrayObject)} marginTop="50px" boxShadow="0px 5px 10px 0px rgba(0,0,0,0.3)" cursor="pointer">
      ...
      {missingHover == arrayObject.title && 
        <Box position="absolute" bg="rgba(255,255,250,0.9)" width="170px" height="180px" top="0" borderRadius="25px" p="15px" overflowY="scroll">
          ...
          {typeHover == "cookbookView" && <>
            <Flex flexDirection="column" width="170px" height="180px" p="15px" justifyContent="space-between" alignItems="center">
